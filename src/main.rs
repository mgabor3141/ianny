...
fn execute_action(
    ...
    match action {
        ...
        Action::ShowBreakCountdown {
            ...
            let (elapsed, was_interrupted) = show_timed_notification(
                &summary,
                &body,
                Duration::from_secs(duration),
                &check_resumed,
...
fn main() -> ! {
    ...
    std::thread::spawn(move || -> ! {
        ...
        let mut last_wallclock = SystemTime::now();
        ...
        loop {
            ...
            let monotonic_elapsed = now_monotonic.duration_since(last_monotonic).as_secs();
            // Wall-clock elapsed detects suspend: SystemTime advances
            // during sleep while Instant (CLOCK_MONOTONIC) does not.
            let wallclock_elapsed = now_wallclock
                .duration_since(last_wallclock)
                .unwrap_or_else(|_| Duration::from_secs(monotonic_elapsed))
                .as_secs();
            last_monotonic = now_monotonic;
            last_wallclock = now_wallclock;

            // Detect system suspend/sleep: if wall-clock jumped far beyond
            // what the monotonic clock measured, the system was asleep.
            let suspend_gap = wallclock_elapsed.saturating_sub(monotonic_elapsed);
            let min_idle = CONFIG
                .timer
                .breaks
                .iter()
            ...
            if suspend_gap >= min_idle {
                info!(
                    "System wake detected: wall-clock jumped {wallclock_elapsed}s \
                     vs monotonic {monotonic_elapsed}s (gap {suspend_gap}s) — \
                     treating as break, resetting timers"
                );
