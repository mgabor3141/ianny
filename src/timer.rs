...
pub struct TimerEngine {
    ...
    /// Whether we've already retried a nudge after an early interruption
    /// in the current slot.
    retried_nudge_this_slot: bool,
}
...
impl TimerEngine {
    ...
    pub fn tick(&mut self, elapsed: u64, config: &Config, inhibitors_active: bool) -> Action {
        ...
        if self.active_time >= config.timer.base_interval {
            ...
            self.retried_nudge_this_slot = false;
    ...
    /// If interruption happens early (first half), retry with one immediate nudge.
    pub fn break_interrupted(
        &mut self,
        tier_index: usize,
        elapsed: u64,
        config: &Config,
    ) -> Action {
        let tier = &config.timer.breaks[tier_index];
        let retry_window = tier.idle_threshold / 2;

        if elapsed < retry_window && !self.retried_nudge_this_slot {
            self.retried_nudge_this_slot = true;
            self.pending_nudge_tier = Some(tier_index);
            let (summary, body) = tier.message(self.skip_count);

            info!(
                "Break interrupted early at {elapsed}s (< {}s) — retrying nudge once",
                retry_window
            );

            return Action::ShowNudge {
                tier_index,
                summary: summary.to_owned(),
                body: body.to_owned(),
                nudge_duration: tier.nudge_duration,
...
mod tests {
    ...
    #[test]
    fn test_break_interrupted_early_retries_nudge_once() {
        let config = test_config();
        let mut engine = TimerEngine::new();

        // Take a break then interrupt very early (< half of 20s threshold)
        let _ = engine.tick(100, &config, false);
        let _ = engine.nudge_result(true, &config);
        let action = engine.break_interrupted(0, 5, &config);
        assert!(matches!(action, Action::ShowNudge { tier_index: 0, .. }));

        // Second early interruption in same slot should not retry again
        let action = engine.break_interrupted(0, 5, &config);
        assert!(matches!(action, Action::ShowFeedback { .. }));
    }

    #[test]
    fn test_break_interrupted_late_keeps_skip_count() {
        let config = test_config();
        let mut engine = TimerEngine::new();

        // Take a break but interrupt after retry window (>= half)
        let _ = engine.tick(100, &config, false);
        let _ = engine.nudge_result(true, &config);
        let action = engine.break_interrupted(0, 15, &config);
        assert!(matches!(action, Action::ShowFeedback { .. }));
